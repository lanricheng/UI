...
public class SlidingLayout extends LinearLayout implements View.OnTouchListener {
    ...
    private int leftEdge;          //左侧布局最多可以滑动到的左边缘。值由左侧布局的宽度来定，
    // marginLeft到达此值之后，不能再减少。

    private int rightEdge = 0;    //左侧布局最多可以滑动到的右边缘。值恒为0，
    // 即marginLeft到达0之后，不能增加。

    private int leftLayoutPadding = 220;     //左侧布局完全显示时，留给右侧布局的宽度值。
    ...
    // 通过此参数来重新确定左侧布局的宽度，以及更改leftMargin的值。
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                // 手指按下时，记录按下时的横坐标
                xDown = event.getRawX();
                if(xDown>screenWidth-250){
                    scrollToRightLayout();
                    sleep(100);
                }
                if(isLeftLayoutVisible){                 //如果左侧布局完全显示
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isLeftLayoutVisible) {                             //如果左布局全部显示
                    leftLayoutParams.leftMargin = distanceX;
                } else {
                    leftLayoutParams.leftMargin = leftEdge + distanceX;
                }
                if (leftLayoutParams.leftMargin < leftEdge) {           //如果左侧布局显示没有完全
                    leftLayoutParams.leftMargin = leftEdge;             //左侧布局显示完全
                } else if (leftLayoutParams.leftMargin > rightEdge) {   //如果左侧布局将要显示过大
                    leftLayoutParams.leftMargin = rightEdge;            //保持完全显示
                }
                leftLayout.setLayoutParams(leftLayoutParams);
                break;
            case MotionEvent.ACTION_UP:
                ...
                xUp = event.getRawX();
                if (wantToShowLeftLayout()) {                //如果往右边滑且速度足够
                    if (shouldScrollToLeftLayout()) {        //如果移动距离够或速度足够
                        scrollToLeftLayout();                //显示左边布局
                    } else {
                        scrollToRightLayout();
                    }
                } else if (wantToShowRightLayout()) {        //如果往左边滑且速度足够
